...
class Context {
    ...
    /**
     * Wait for the native context to be loaded
     */
    waitForNativeContextLoaded () {
        driver.waitUntil(
            () => {
                const currentContexts = this.getCurrentContexts();

                return currentContexts.length > 0 &&
                    currentContexts.find(context => context.toLowerCase().includes(CONTEXT_REF.NATIVE));
            },
            10000,
            'Webview context not loaded',
            100
